...
export async function PATCH(
  req: Request,
  { params }: { params: { classId: string; quizId: string } }
) {
  try {
...
return NextResponse.json(question);
    ...
    //   quizQuestion = await db.quizQuestion.update({
    //     where: {
    ...
    //   });
    // } else {
    //   // Create new question and link it to the quiz
    //   quizQuestion = await db.quizQuestion.create({
    //     data:{
    //       ...values
    //     } 
    //   });
